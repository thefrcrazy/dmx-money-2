...
const QifImportModal: React.FC<QifImportModalProps> = ({ isOpen, onClose, file, onImport }) => {
    ...
    const handleNext = () => {
        if (currentStep === 'preview') {
            ...
            if (uniqueQifCategories.length > 0) {
                // Pre-fill mapping with exact matches
                const newMapping = { ...categoryMapping };
                uniqueQifCategories.forEach(qifCat => {
                    const match = categories.find(c => c.name.toLowerCase() === qifCat.toLowerCase());
                    if (match) newMapping[qifCat] = match.id;
                    else newMapping[qifCat] = 'new'; // Default to create new
                });
    ...
    const handleBack = () => {
        if (currentStep === 'account') setCurrentStep('preview');
            ...
            if (uniqueQifCategories.length > 0) setCurrentStep('categories');
    ...
    const handleFinalImport = async () => {
        ...
        try {
            ...
            for (const qifCat of Object.keys(categoryMapping)) {
                if (categoryMapping[qifCat] === 'new') {
                    const newCatId = await addCategory({
                        name: qifCat,
                        icon: 'Tag',
                        color: '#9ca3af'
                    });
                    finalCategoryMapping[qifCat] = newCatId;
                }
            }
                ...
                return (
                    <div className="space-y-6">
                        <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg flex items-start gap-3">
                            ...
                            <div>
                                <h4 className="font-medium text-blue-900 dark:text-blue-100">Aperçu du fichier QIF</h4>
                ...
                return (
                    <div className="space-y-4">
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                            Associez les catégories du fichier QIF à vos catégories existantes.
                        </p>
                        <div className="max-h-[400px] overflow-y-auto pr-2 space-y-2">
                            {uniqueQifCategories.map(qifCat => (
                                <div key={qifCat} className="flex items-center gap-4 p-3 bg-gray-50 dark:bg-neutral-800 rounded-lg">
                                    <div className="flex-1 font-medium text-sm text-gray-900 dark:text-gray-200 truncate" title={qifCat}>
                                        {qifCat}
                                    </div>
                                    <ArrowRight className="w-4 h-4 text-gray-400" />
                                    <div className="flex-1 min-w-[180px]">
                                        <select
                                            value={categoryMapping[qifCat] || 'new'}
                                            onChange={(e) => setCategoryMapping(prev => ({ ...prev, [qifCat]: e.target.value }))}
                                            className="w-full text-sm rounded-md border-gray-300 dark:border-neutral-600 dark:bg-neutral-700"
                                        >
                                            <option value="new">+ Créer "{qifCat}"</option>
                                            {categories.map(c => (
            ...
            <div className="app-card w-full max-w-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-200 app-modal-content app-import-wizard-modal">
                ...
                <div className="p-6 border-b border-gray-100 dark:border-neutral-800 flex items-center justify-between app-modal-header">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 app-modal-title">Assistant d'import QIF</h3>
